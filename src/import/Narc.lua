...
local function parseBlocks(reader, declaredSize, blockCount)
  ...
  for _ = 1, blockCount do
    if offset + BLOCK_HEADER_SIZE > declaredSize then
      Errors.raise("NARC_BLOCK_PAST_END",
...
local function parseMembers(reader, btaf, gmif)
  ...
  for memberId = 0, count - 1 do
    ...
    if startOffset > endOffset or endOffset > gmif.payloadLength then
      Errors.raise("NARC_MEMBER_OUT_OF_RANGE",
        "member " .. memberId .. " range [" .. startOffset .. ", " .. endOffset
          .. ") exceeds GMIF payload of " .. gmif.payloadLength,
        { memberId = memberId, startOffset = startOffset, endOffset = endOffset,
          gmifSize = gmif.payloadLength })
    end
    members[memberId + 1] = {
      memberId = memberId,
...
local function parse(data, label)
  ...
  if reader:length() < HEADER_SIZE then
    Errors.raise("NARC_TRUNCATED", "NARC is " .. reader:length() .. " bytes, need at least " .. HEADER_SIZE,
      { size = reader:length() })
  end
  ...
  if not byMagic.BTAF then Errors.raise("NARC_MISSING_BTAF", "NARC has no BTAF block") end
  if not byMagic.GMIF then Errors.raise("NARC_MISSING_GMIF", "NARC has no GMIF block") end
...
function Narc:memberInfo(memberId)
  ...
  if not m then
    return nil, Errors.new("NARC_MEMBER_ID_OUT_OF_RANGE",
...
function Narc.detectCompression(data)
  ...
  if kind == 0x10 then return "lz10" end
  if kind == 0x11 then return "lz11" end
  return nil
end
