...
function T.decrypts_known_synthetic_bank()
  ...
  local second = { 0x013A, 0x0156, 0x0153, 0x014A, 0x0149, 0x0157, 0x0157, 0x0153,
...
function T.corrupt_entry_offsets_are_typed()
  ...
  local corrupted = bytes:sub(1, 3) .. string.char(
    encBad % 256, math.floor(encBad / 256) % 256,
    math.floor(encBad / 65536) % 256, math.floor(encBad / 16777216) % 256) .. bytes:sub(9)
...
function T.unaligned_and_zero_length_entries_are_typed()
  ...
  local bytes = string.char(1, 0, 0, 0) .. string.char(
    encBad % 256, math.floor(encBad / 256) % 256,
    math.floor(encBad / 65536) % 256, math.floor(encBad / 16777216) % 256,
    0, 0, 0, 0)
  returnsCode("MESSAGE_ENTRY_OUT_OF_BOUNDS", function()
    return FieldMessageBank.decode(bytes, {})
  end)
...
function T.overlapping_entries_are_typed()
  ...
  local key = 0x2222
  local bytes = FieldMessageBank.encodeForTests(
    { { 0x0041, 0x0042, 0xFFFF }, { 0x0043, 0xFFFF } }, key)
  ...
  local entry1 = string.char(
    encOffset % 256, math.floor(encOffset / 256) % 256,
    math.floor(encOffset / 65536) % 256, math.floor(encOffset / 16777216) % 256,
    encLength % 256, math.floor(encLength / 256) % 256,
    math.floor(encLength / 65536) % 256, math.floor(encLength / 16777216) % 256)
...
function T.missing_eos_is_typed()
  ...
  local encOffset = xor32(offset, seed32)
  local encLength = xor32(length, seed32)
  local member = string.char(1, 0, key % 256, math.floor(key / 256)) .. string.char(
    encOffset % 256, math.floor(encOffset / 256) % 256,
    math.floor(encOffset / 65536) % 256, math.floor(encOffset / 16777216) % 256,
    encLength % 256, math.floor(encLength / 256) % 256,
    math.floor(encLength / 65536) % 256, math.floor(encLength / 16777216) % 256)
