-- Parser and validator for an HGSS field-light profile text table (e.g.
-- data/area00light.txt). Each record is a time threshold, four directional
-- light slots (enable + BGR555 color + fx12 vector), and four material colors
-- (diffuse/ambient/specular/emission). The field engine picks the active
-- record from the time of day; runtime selection over the parsed records is
-- FieldLightProfile's contract (libs/assets), which consumes only the
-- normalized records. This module is pure: it takes the file text (LF or
-- CRLF) and returns normalized integer records; hashing/IO stays with the
-- caller. No source text grammar, file path, or HGSS parsing error belongs in
-- the runtime half. Source: the field light tables and loader in
-- pret/pokeheartgold.
