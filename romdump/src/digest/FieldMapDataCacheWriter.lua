...
local function persist(cacheFs, bundle)
  ...
  if not field or not dependencies then
    Errors.raise("FIELD_MAP_DATA_CACHE_STALE",
  ...
  if field.schema ~= "g4-field-map-v1" or field.mapId ~= mapId then
    Errors.raise("FIELD_MAP_DATA_CACHE_STALE", "field-map readback has the wrong identity",
...
function FieldMapDataCacheWriter.write(cacheFs, bundle)
  assert(cacheFs and type(bundle) == "table" and bundle.mapId and bundle.marker,
    "invalid field-map bundle")
  local ok, result = pcall(persist, cacheFs, bundle)
  if ok then return result end
  pcall(function() FieldMapDataCache.invalidateMap(cacheFs, bundle.mapId) end)
  error(result)
end
