...
local function _discover(overlayBytes, overlayInfo, config)
  ...
  for _, offset in ipairs(config.pointerFileOffsets) do
    if offset < 0 or offset + 4 > #overlayBytes then
      Errors.raise(
        "FIELD_CAMERA_POINTER_OUT_OF_BOUNDS",
  ...
  if config.expectedTableRamAddress and tableRamAddress ~= config.expectedTableRamAddress then
    Errors.raise(
      "FIELD_CAMERA_TABLE_ADDRESS_UNEXPECTED",
      string.format("camera table address 0x%08X ~= expected 0x%08X", tableRamAddress, config.expectedTableRamAddress),
  ...
  if tableFileOffset < 0 or tableFileOffset + tableByteLength > #overlayBytes then
    Errors.raise(
      ...
      "discovered camera table is outside its overlay",
      { tableFileOffset = tableFileOffset, tableByteLength = tableByteLength, overlayByteLength = #overlayBytes }
    )
  end
  return {
