...
T.wide_host_moves_oak_into_the_profile_region_before_selection = function(scope)
  ...
  Assert.isNil(first.layout.genderChoiceGroup)
  ...
  for frame = 0, 26 do
    ...
    if frame < 26 then
      Assert.isNil(view.layout.genderChoiceGroup)
  ...
  Assert.notNil(final.layout.genderChoiceGroup)
...
T.resized_tall_host_keeps_the_completed_profile_composition = function(scope)
  ...
  Assert.notNil(completed.layout.genderChoiceGroup)
  ...
  Assert.notNil(reentered.layout.genderChoiceGroup)
end
