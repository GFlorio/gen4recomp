...
function T.the_demo_walk_to_the_door_warps_to_town_and_back(romFs)
  ...
  -- The step into the warp cell (4,14) is walkable; the tile is
  -- WARP_ENTRANCE_SOUTH, so the warp itself fires on the facing path (idle,
  -- facing south) rather than on the step path.
  ...
  local labTrigger = assert(TransitionTrigger.inputPath(lab, 4, 14, "south"), "facing south on the lab door resolves")
  Assert.equal(labTrigger.kind, "directional")
  Assert.equal(labTrigger.behavior, 101)
  manager:dispose()

  -- Town arrival: the door cell (684,393) is a blocked DOOR metatile with a
  -- warp behind it, so the return trip uses the FACING path: step south off
  ...
  local facingTrigger = assert(TransitionTrigger.inputPath(town, 684, 394, "north"), "facing the town door resolves")
  Assert.equal(facingTrigger.kind, "door")
  Assert.equal(facingTrigger.behavior, 105)
  Assert.equal(assert(facingTrigger.warp).destinationMapId, 61)
