...
| Movement command catalog (code → macro name, u16 arg count) | `data/reference/hgss/movement_commands.lua` | `pret/pokeheartgold`: `include/constants/movements.h`, `asm/macros/movement.inc` |
| Standard-script catalog (CallStd id → public name, script-bank mapping) | `data/reference/hgss/std_script_catalog.lua`, `romdump/src/digest/script/SourceCatalog.lua` | `pret/pokeheartgold`: `include/constants/std_script.h`, `src/script_manager.c` (`sScriptBankMapping`) |
| Script-member message banks (NPCMsg indices resolve through the owning map's bank) | `data/reference/hgss/script_members.lua` | `pret/pokeheartgold`: `src/data/map_headers.h`, `src/script_manager.c`; cross-checked against the per-member `.s` includes |
| Sound-sequence, flag, and variable catalogs (numeric operands → decomp names) | `data/reference/hgss/sndseq.lua`, `flags.lua`, `vars.lua` | `pret/pokeheartgold`: `include/constants/sndseq.h`, `flags.h`, `vars.h` |
