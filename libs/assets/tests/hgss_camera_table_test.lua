...
local function record(r)
  return u32(r.distance) .. u16(r.angleX) .. u16(r.angleY or 0)
    .. u16(r.angleZ or 0) .. u16(r.unknownAngle or 0)
    .. u8(r.projection) .. u8(r.unknownByte or 0) .. u16(r.halfAngle)
    .. u32(r.near) .. u32(r.far) .. u32(r.offsetX or 0)
    .. u32(r.offsetY or 0) .. u32(r.offsetZ or 0)
end

local TYPE0 = record({ distance = 0x0029AEC1, angleX = 0xDD62,
  projection = 0, halfAngle = 0x05C1, near = 0x00096000, far = 0x004B0000 })
local TYPE4 = record({ distance = 0x0061B89B, angleX = 0xDC82,
  projection = 1, halfAngle = 0x0281, near = 0x00096000, far = 0x006C7000 })
...
function T.decodes_frozen_type_zero_and_four_values()
  local filler = record({ distance = 0, angleX = 0, projection = 0,
    halfAngle = 0, near = 0, far = 0 })
  local bytes = TYPE0 .. filler .. filler .. filler .. TYPE4
  local table = assert(HgssCameraTable.decode(bytes,
    { tableOffset = 0, recordCount = 5, source = "test-overlay" }))
  ...
  near(table.records[0].fullVerticalFovRadians,
    math.rad(table.records[0].fullVerticalFovDegrees), 0.000000001)
...
function T.preserves_signed_raw_fields_and_normalizes_offsets()
  local bytes = record({ distance = 0x10000, angleX = 0xFFFF, angleY = 0x8000,
    angleZ = 0x7FFF, unknownAngle = 0xFFFE, projection = 0,
    halfAngle = 0x4000, near = 0x1000, far = 0x2000,
    offsetX = 0xFFFF0000, offsetY = 0x10000, offsetZ = 0x80000000 })
  local profile = assert(HgssCameraTable.decode(bytes,
    { tableOffset = 0, recordCount = 1, source = "signed" })).records[0]
...
function T.rejects_bad_projection_and_truncated_table()
  local bad = record({ distance = 0, angleX = 0, projection = 2,
    halfAngle = 0, near = 0, far = 0 })
