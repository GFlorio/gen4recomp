...
local FONT = { charmap = {
  ["P"] = 0x013A, ["r"] = 0x0156, ["o"] = 0x0153, ["f"] = 0x014A, ["e"] = 0x0149,
  ["s"] = 0x0157, ["m"] = 0x0151, ["E"] = 0x012F, ["l"] = 0x0150, ["n"] = 0x0152,
  ["G"] = 0x0131, ["O"] = 0x0139, ["L"] = 0x0138, ["D"] = 0x012E, ["i"] = 0x014D,
  ["H"] = 0x0132, ["I"] = 0x0133, ["v"] = 0x015A, ["b"] = 0x0146, ["g"] = 0x014B, ["t"] = 0x0158,
  ["k"] = 0x014F, ["c"] = 0x0147, ["u"] = 0x0159, ["w"] = 0x015B, ["a"] = 0x0145,
  ["4"] = 0x0125, ["9"] = 0x012A,
  ["!"] = 0x01AB, [" "] = 0x01DE, ["é"] = 0x0188, ["’"] = 0x01B3,
  [":"] = 0x01C4, [","] = 0x01AD,
} }
...
function T.marker_builds_gmm_style_markers()
  Assert.equal(FieldMessageText.marker(FieldMessageText.STRVAR_1 + 3, 0, 0),
    "{STRVAR_1 3, 0, 0}")
...
function T.parse_reads_marker_text_back_into_tokens()
  local tokens = assert(FieldMessageText.parse(
    "Professor Elm: Hi, {STRVAR_1 3, 0, 0}!\nI’ve been waiting!\r{CURSOR_X 4}",
    FONT))
  Assert.equal(tokens[1].kind, "glyph")
  Assert.equal(tokens[1].code, 0x013A)
  local strvar, cursor, breaks = nil, nil, {}
  for _, token in ipairs(tokens) do
    if token.kind == "substitution" and token.control == 0x0103 then strvar = token end
    if token.kind == "unsupported_control" and token.control == 0x0203 then cursor = token end
...
function T.nothing_is_dropped_from_the_text_form()
  ...
  Assert.equal(FieldMessageText.tokensToText(tokens),
