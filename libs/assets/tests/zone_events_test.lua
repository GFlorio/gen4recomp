...
function T.reports_the_first_truncated_record_source_offset()
  ...
  Assert.isNil(result)
  Assert.equal(err.code, "ZONE_EVENTS_TRUNCATED")
  Assert.equal(err.context.category, "backgroundEvents")
  Assert.equal(err.context.recordIndex, 0)
  Assert.equal(err.context.sourceOffset, 4)
end
...
function T.rejects_or_optionally_preserves_trailing_bytes()
  ...
  Assert.isNil(result)
  Assert.equal(err.code, "ZONE_EVENTS_TRAILING_BYTES")
  Assert.equal(err.context.sourceOffset, 16)
