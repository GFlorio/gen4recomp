...
--
-- Spec requirement: section 18 (alpha classifier v2). Tests verify the
-- new MIXED class and final-alpha-aware classification logic driven by
-- polygon mode and alpha usage, not format alone.
...
-- MIXED with A3I5 and A5I3 formats (spec requirement: "At least one MIXED test
-- must use A3I5 and one A5I3 source format, even though format is no longer
-- the sole rule").
...
-- DECAL tests (spec requirement: "DECAL tests are mandatory. Omitting them
-- leaves the most important classifier nuance unprotected").
--
