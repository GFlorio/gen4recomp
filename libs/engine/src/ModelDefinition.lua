...
--     key = "fixture:door",
--     sourceBackend = "generic",        -- "nitro" | "generic"
...
local function validateVec3(value, what)
  if type(value) ~= "table" or not isFiniteNumber(value.x) or not isFiniteNumber(value.y)
    or not isFiniteNumber(value.z) then
    Errors.raise("MODEL_DEF_BAD_TRANSFORM",
      "node " .. what .. " must be { x, y, z } finite numbers", { what = what })
  end
end
...
local function validateNodes(nodes)
  for i, node in ipairs(nodes) do
    ...
    if node.index ~= index then
      Errors.raise("MODEL_DEF_NODE_INDEX_MISMATCH",
        "nodes must be contiguous zero-based indices; node " .. i .. " has index "
          .. tostring(node.index), { nodeIndex = node.index, expected = index })
    end
    if node.parentIndex ~= nil then
      if not isInteger(node.parentIndex) or node.parentIndex < 0 or node.parentIndex >= index then
        Errors.raise("MODEL_DEF_BAD_PARENT",
    ...
    if type(node.rotation) ~= "table" or #node.rotation ~= 9 then
      Errors.raise("MODEL_DEF_BAD_ROTATION",
        "node " .. index .. " rotation must be a 9-cell matrix", { nodeIndex = index })
    end
    for k = 1, 9 do
      if not isFiniteNumber(node.rotation[k]) then
        Errors.raise("MODEL_DEF_BAD_ROTATION",
          "node " .. index .. " rotation cell " .. k .. " is not finite", { nodeIndex = index })
      end
    end
  end
end
...
local function validateMeshes(meshes, nodeCount, materialCount)
  for i, mesh in ipairs(meshes) do
    if type(mesh.id) ~= "string" or #mesh.id == 0 then
      Errors.raise("MODEL_DEF_MESH_NO_ID",
        "mesh " .. i .. " requires a non-empty id", {})
    end
    if not (isInteger(mesh.nodeIndex) and mesh.nodeIndex >= 0 and mesh.nodeIndex < nodeCount) then
      Errors.raise("MODEL_DEF_MESH_BAD_NODE",
        "mesh " .. mesh.id .. " references an unknown node index",
        { meshId = mesh.id, nodeIndex = mesh.nodeIndex })
    end
    if not (isInteger(mesh.materialIndex) and mesh.materialIndex >= 0
      and mesh.materialIndex < materialCount) then
      Errors.raise("MODEL_DEF_MESH_BAD_MATERIAL",
    ...
    if type(mesh.batch) ~= "table" or type(mesh.batch.vertices) ~= "table" then
      Errors.raise("MODEL_DEF_MESH_NO_BATCH",
        "mesh " .. mesh.id .. " requires a batch with vertices", { meshId = mesh.id })
    end
  end
end
...
local function validateMaterials(materials)
  for i, material in ipairs(materials) do
    if not isInteger(material.id) or material.id ~= i - 1 then
      Errors.raise("MODEL_DEF_MATERIAL_INDEX_MISMATCH",
        "materials must be contiguous zero-based indices; material " .. i .. " has id "
          .. tostring(material.id), {})
    end
    if not ModelDefinition.ALPHA_MODES[material.alphaMode] then
      Errors.raise("MODEL_DEF_BAD_ALPHA_MODE",
    ...
    if type(base) ~= "table" or not isInteger(base.r) or not isInteger(base.g)
      or not isInteger(base.b) or not isInteger(base.a)
      or base.r < 0 or base.r > 255 or base.g < 0 or base.g > 255
      or base.b < 0 or base.b > 255 or base.a < 0 or base.a > 255 then
      Errors.raise("MODEL_DEF_BAD_BASE_COLOR",
...
local function validateAnimations(animations)
  for _, clip in ipairs(animations) do
    if type(clip) ~= "table" or clip.id == nil then
      Errors.raise("MODEL_DEF_BAD_ANIMATION",
        "animations must be AnimationClip values", {})
    end
  end
end
...
function ModelDefinition.new(spec)
  ...
  if not ModelDefinition.SOURCE_BACKENDS[spec.sourceBackend] then
    Errors.raise("MODEL_DEF_BAD_SOURCE_BACKEND",
  ...
  if spec.skins then validateSkins(spec.skins, #spec.nodes) end
  if spec.animations then validateAnimations(spec.animations) end
  ...
  for _, clip in ipairs(spec.animations or {}) do
    if byName[clip.name] then
      Errors.raise("MODEL_DEF_DUPLICATE_ANIMATION",
        "model " .. spec.key .. " has two clips named " .. clip.name, { name = clip.name })
    end
    byName[clip.name] = clip
    for _, semantic in ipairs(clip.semanticNames) do
      if bySemantic[semantic] then
        Errors.raise("MODEL_DEF_DUPLICATE_SEMANTIC",
...
function ModelDefinition:animationNames()
  local out = {}
  for _, clip in ipairs(self.animations) do out[#out + 1] = clip.name end
  return out
end
...
function ModelDefinition:bindingMap(clip)
  ...
  if clip.category == "joint" or clip.category == "visibility" then
    for _, track in ipairs(clip.tracks) do
      ...
      if type(target) == "number" and self:node(target) then map[target] = target end
    end
