...
local function collision(blocked)
  return {
    ...
    getLocal = function(_, x, z) return { x = x, z = z, hardBlocked = false } end,
  }
end

local function flatTerrain(height)
  return TerrainSurface.new({ schema = "g4-terrain-surfaces-v1", plates = {
    { id = 0, minX = 0, minZ = 0, maxX = 32, maxZ = 32,
      normal = { x = 0, y = 1, z = 0 }, distance = height,
...
function T.routes_collision_to_neighbor_cell_coordinates()
  local region = FieldRegion.new(collision(), flatTerrain(0), {
    { offsetTilesX = 32, offsetTilesZ = 0,
      collision = collision({ ["0:4"] = true }), terrain = flatTerrain(0) },
  })
...
function T.translates_neighbor_surfaces_and_connects_shared_edge()
  local region = FieldRegion.new(collision(), flatTerrain(2), {
    { offsetTilesX = 32, offsetTilesZ = 0,
      collision = collision(), terrain = flatTerrain(2) },
  })
  ...
  local resolved = SurfaceResolver.new(region.terrain):resolve({
    localX = 32.5, localZ = 4.5,
    currentSurfaceId = 0, currentY = 2,
...
function T.translates_sloped_plane_distance_without_changing_height()
  local slope = TerrainSurface.new({ plates = {
    { id = 4, minX = 0, minZ = 0, maxX = 32, maxZ = 32,
      normal = { x = -1, y = 1, z = 0 }, distance = 0,
      slopeClass = "ramp-east", walkable = true },
  } })
  local region = FieldRegion.new(collision(), flatTerrain(0), {
    { offsetTilesX = -32, offsetTilesZ = 32,
      collision = collision(), terrain = slope },
  })
