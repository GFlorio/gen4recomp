...
local function jointClip()
  return AnimationClip.new({
    ...
    tracks = { { target = 1, channels = { rotation = { interpolation = "linear",
...
local function definitionSpec()
  return {
    ...
    nodes = {
      { index = 0, name = "root", translation = { x = 0, y = 0, z = 0 },
        rotation = { 1, 0, 0, 0, 1, 0, 0, 0, 1 }, scale = { x = 1, y = 1, z = 1 } },
      { index = 1, name = "leaf", parentIndex = 0, translation = { x = 1, y = 0, z = 0 },
        rotation = { 1, 0, 0, 0, 1, 0, 0, 0, 1 }, scale = { x = 1, y = 1, z = 1 } },
    },
    meshes = {
      { id = "m0", name = "m0", nodeIndex = 1, materialIndex = 0,
...
function T.validation_rejects_bad_shapes()
  ...
  s.sourceBackend = "vrm"
  throwsCode("MODEL_DEF_BAD_SOURCE_BACKEND", function() return ModelDefinition.new(s) end)
  s = definitionSpec()
  s.materials[1].alphaMode = "pbr"
  throwsCode("MODEL_DEF_BAD_ALPHA_MODE", function() return ModelDefinition.new(s) end)
  s = definitionSpec()
  s.nodes[2].index = 2
  throwsCode("MODEL_DEF_NODE_INDEX_MISMATCH", function() return ModelDefinition.new(s) end)
  s = definitionSpec()
  s.nodes[2].parentIndex = 5
  throwsCode("MODEL_DEF_BAD_PARENT", function() return ModelDefinition.new(s) end)
  s = definitionSpec()
  s.meshes[1].nodeIndex = 7
  throwsCode("MODEL_DEF_MESH_BAD_NODE", function() return ModelDefinition.new(s) end)
  s = definitionSpec()
  s.meshes[1].materialIndex = 7
  throwsCode("MODEL_DEF_MESH_BAD_MATERIAL", function() return ModelDefinition.new(s) end)
  s = definitionSpec()
  s.materials[1].baseColor = { r = 300, g = 0, b = 0, a = 255 }
  throwsCode("MODEL_DEF_BAD_BASE_COLOR", function() return ModelDefinition.new(s) end)
  s = definitionSpec()
  s.animations[1].name = "open"
  s.animations[2] = s.animations[1]
  throwsCode("MODEL_DEF_DUPLICATE_ANIMATION", function() return ModelDefinition.new(s) end)
  s = definitionSpec()
  ...
  throwsCode("MODEL_DEF_DUPLICATE_SEMANTIC", function() return ModelDefinition.new(s) end)
end
...
function T.binding_map_maps_node_and_material_targets()
  ...
  local material = AnimationClip.new({
    id = "c2", name = "fade", category = "material", kind = "color",
    frameCount = 4,
    tracks = { { target = "wall", channels = { diffuse = { interpolation = "step",
      keys = { { frame = 0, value = 0x203C } } } } } },
  })
  Assert.deepEqual(def:bindingMap(material), { wall = 0 })

  local unmapped = AnimationClip.new({
    id = "c3", name = "ghost", category = "material", kind = "color",
    frameCount = 4,
    tracks = { { target = "absent", channels = { diffuse = { interpolation = "step",
