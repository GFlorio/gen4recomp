...
-- A pose can retain the source-side matrix-boundary provenance while the
-- renderer-facing item must use the ordinary current draw transform.
function T.straddle_origin_items_use_the_current_transform_and_normal()
  ...
  local previous = Matrix4.translate(40, 0, 0)
  draw.position = current
  draw.straddle = { leading = 2, position = previous }
  ...
  ---@diagnostic disable-next-line: undefined-field -- proves discarded provenance is not exported
