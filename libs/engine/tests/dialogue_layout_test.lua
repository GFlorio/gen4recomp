...
local function textOf(line)
  ...
  for _, token in ipairs(line.tokens) do
    if token.kind == "glyph" then out[#out + 1] = token.text end
  end
...
function T.wraps_on_width_and_prefers_breakable_spaces()
  ...
  local layout = DialogueLayout.layout(glyphs("AAAA BBBB"), metrics(FONT),
    { width = 24, maxLines = 2 })
...
function T.full_page_overflows_to_a_new_page()
  ...
  local more = DialogueLayout.layout(glyphs("AAAA BBBB CCCC DDDD EEEE"), metrics(FONT),
    { width = 24, maxLines = 2 })
...
function T.explicit_line_break_starts_a_line_and_flushes_at_capacity()
  local function withBreaks(count)
    ...
    for _ = 1, count do
      ...
      position = position + 1
      for i = position, position + 3 do tokens[i] = tokens[i - position + 1] end
      position = position + 4
    end
    return tokens
  end
  local layout = DialogueLayout.layout(withBreaks(1), metrics(FONT),
    { width = 24, maxLines = 2 })
  Assert.equal(#layout.pages, 1)
  Assert.equal(textOf(layout.pages[1].lines[1]), "AAAA")
  ...
  local three = DialogueLayout.layout(withBreaks(2), metrics(FONT),
    { width = 24, maxLines = 2 })
...
function T.style_and_unresolved_tokens_take_no_layout_width()
  local tokens = {
    ...
    { kind = "glyph", code = 0x0121, text = "A", raw = { 0x0121 } },
    { kind = "substitution", control = 0x0103, args = { 0, 0 },
...
function T.empty_and_control_only_messages_close_safely()
  local empty = DialogueLayout.layout(
    { { kind = "eos", raw = { 0xFFFF } } }, metrics(FONT), { width = 24 })
...
function T.missing_glyph_advance_is_typed()
  local err = Assert.throws(function()
    return DialogueLayout.layout(
      { { kind = "glyph", code = 0x0001, text = "?", raw = { 0x0001 } },
        { kind = "eos", raw = { 0xFFFF } } }, metrics(FONT), { width = 24 })
  end)
