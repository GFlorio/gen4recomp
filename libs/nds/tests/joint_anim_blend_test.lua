...
-- Nitro's fxMul64: (a * b) >> 12 with 32-bit signed wrap.
...
function T.single_contribution_is_copied()
  ...
  local out = assert(JointAnimBlend.blend({ { ratio = 0x1000, result = r } }))
...
function T.unequal_ratios_normalize_with_fx_div()
  ...
  local out = assert(JointAnimBlend.blend({ { ratio = 0x800, result = a }, { ratio = 0x400, result = b } }))
...
function T.translation_uses_64bit_fx_mul()
  ...
  local b = result()
  local out = assert(JointAnimBlend.blend({ { ratio = 0x1000, result = a }, { ratio = 0x1000, result = b } }))
  -- total 0x2000, weights 0x800 each.
  Assert.equal(out.trans[1], fxMul64(0x800, 0x7FFFFFFF), "64-bit FX_Mul keeps the middle bits")
  Assert.equal(out.scale[1], mul32Path(0x800, 0x7FFFFFFF) + 0x800, "32-bit path takes the low bits, then shifts")
end
...
function T.from_model_rotation_accumulates_weight_on_two_cells()
  ...
  local control = assert(JointAnimBlend.blend({ { ratio = 0x1000, result = b } }))
...
function T.flags_are_and_combined()
  ...
  Assert.equal(assert(JointAnimBlend.blend({ { ratio = 1, result = result({ flags = 0 }) } })).flags, 0)
end
...
function T.blended_rotation_is_orthonormalized()
  ...
  local out = assert(JointAnimBlend.blend({
    { ratio = 0x800, result = identity },
    { ratio = 0x800, result = rotY90 },
  }))
