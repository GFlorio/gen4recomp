...
local function withGraphicsSpy(fn)
  ...
  rawset(graphics, "getHeight", function()
    return 480
  end)
  rawset(graphics, "getScissor", function()
    if scissor == nil then
      return nil
    end
    return scissor[1], scissor[2], scissor[3], scissor[4]
  end)
  rawset(graphics, "getWidth", function()
    return 640
  end)
  rawset(graphics, "print", function() end)
  rawset(graphics, "printf", function(text, x, y, width, align)
  ...
  rawset(graphics, "setLineWidth", function() end)
  rawset(graphics, "setScissor", function(x, y, width, height)
    scissor = x == nil and nil or { x, y, width, height }
  end)
  ...
  for _, name in ipairs(names) do
    rawset(graphics, name, original[name])
  end
  if not ok then
    error(err, 0)
  end
...
function T.delete_and_dialog_actions_keep_layout_behavior_with_beveled_rendering()
  ...
  local initial = menu:view()
  local initialDelete = assert(initial.layout.cards["save-00000001"].delete)
  Assert.deepEqual(menu:hitTest(initialDelete.x + 1, initialDelete.y + 1), {
    primary = nil,
  ...
  for _, printCall in ipairs(cardCalls.prints) do
    if printCall.text == "Delete" then
      cardDeleteText = printCall
      break
    end
  end
  Assert.notNil(cardDeleteText, "card Delete label must be drawn")
...
function T.narrow_dialog_action_rectangles_remain_drawable()
  ...
  local delete = assert(menu:view().layout.cards["save-00000001"].delete)
  menu:mousepressed(delete.x + 1, delete.y + 1, 1)
  Assert.notNil(menu:view().dialog)
  withGraphicsSpy(function()
    menu:draw()
  end)
end
