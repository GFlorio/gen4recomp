-- The checked-in initial player profile and gameplay options for the explicit
-- developer fresh-field entry, copied and strictly validated by a fresh field
-- session (see libs/engine/src/FieldPlayerData.lua). The name must be 1..7
-- glyphs encodable by the generated field font, the gender one of the
-- gendered-message values (0 = male, 1 = female), and the text frame index
-- must resolve to an imported dialogue frame style. A resumed session uses
-- the saved player-data bucket, never this manifest. Pure data; no love
-- dependency.

return {
  profile = {
    name = "GOLD",
    gender = 0,
    trainerId = 0,
  },
  options = {
    textFrame = 0,
    textSpeed = "fastest",
  },
}
